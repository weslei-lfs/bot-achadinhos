...
# ══════════════════════════════
# HELPERS — PRODUTOS
# Usa variável de ambiente PRODUTOS_JSON para persistência
# Se não tiver, usa arquivo local como fallback
# ══════════════════════════════
def get_produtos():
    # Tenta ler da variável de ambiente primeiro (persistência no Render)
    dados = os.environ.get("PRODUTOS_DATA")
    if dados:
        try:
...
def save_produtos(p):
    # Salva localmente (funciona enquanto servidor não reinicia)
...
def ping_loop():
    while True:
        try:
            req.get(APP_URL, timeout=10)
            print("🏓 Ping enviado — servidor acordado!")
