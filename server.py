...
def redistribuir_produtos():
    """Distribui produtos uniformemente dentro do período configurado"""
    ...
    minutos_por_dia = fim_min - inicio_min

    if minutos_por_dia <= 0:
        minutos_por_dia = 14 * 60  # padrão 14h
    ...
    total = len(produtos)
    total_minutos = minutos_por_dia * dias

    # Intervalo uniforme entre cada produto
    if total == 1:
        intervalo = 0
    else:
        intervalo = total_minutos / total

    dt_inicio = datetime.now(BRASILIA).replace(hour=hi_h, minute=hi_m, second=0, microsecond=0)

    print(f"Redistribuindo {total} produtos | Período: {hora_inicio_str}-{hora_fim_str} | {dias} dias | Intervalo: {intervalo:.0f}min")

    for i, produto in enumerate(produtos):
        offset_total = i * intervalo
        dia_idx = int(offset_total // minutos_por_dia)
        min_no_dia = offset_total % minutos_por_dia

        hora_real = inicio_min + min_no_dia
        h = int(hora_real) // 60
        m = int(hora_real) % 60

        # Garante que não ultrapassa o fim do dia
        if h * 60 + m >= fim_min:
            h = hi_h
            m = hi_m
    ...
    print(f"✅ {total} produtos redistribuídos com sucesso!")
