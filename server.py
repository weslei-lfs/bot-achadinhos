...
def ig_reels_loop():
    """
    Posta 5 Reels por dia no Instagram em horários espaçados automaticamente.
    Você define o horário do primeiro post — os outros 4 são distribuídos ao longo do dia.
    Ciclo contínuo: quando acabar todos os vídeos, recomeça do zero.
    """
    VIDEOS_POR_DIA = 5
    ...
    while True:
        try:
            ...
            # Conta quantos já foram postados hoje no IG
            postados_hoje = sum(1 for v in videos if f"ig_{v['id']}_{hoje}" in enviados)

            if postados_hoje >= VIDEOS_POR_DIA:
                time.sleep(60)
                continue

            # Pega o horário base do primeiro vídeo (ig_hora do primeiro da lista)
            hora_base_str = videos[0].get("ig_hora", "09:00")
            hb_h, hb_m = map(int, hora_base_str.split(":"))
            inicio_min = hb_h * 60 + hb_m

            # Distribui os 5 horários a partir do horário base até 23:00
            # Ex: base 09:00 → 09:00, 11:48, 14:36, 17:24, 20:12
            fim_min = 23 * 60  # até 23h
            intervalo = (fim_min - inicio_min) // (VIDEOS_POR_DIA - 1) if VIDEOS_POR_DIA > 1 else 0
            horarios_do_dia = []
            for i in range(VIDEOS_POR_DIA):
                ...
                m = min_total % 60
                if h < 24:
                    horarios_do_dia.append(f"{h:02d}:{m:02d}")

            # Verifica se hora atual bate com algum dos horários e ainda não postou
            if hora_atual in horarios_do_dia:
                slot_idx = horarios_do_dia.index(hora_atual)
                chave_slot = f"ig_slot_{hoje}_{slot_idx}"

                if chave_slot not in enviados:
                    # Pega o próximo vídeo do ciclo
                    total = len(videos)
                    # Conta total de posts IG já feitos (ciclo global)
                    total_ig = sum(1 for k in enviados if k.startswith("ig_slot_"))
                    idx = total_ig % total
                    video = videos[idx]
                    ...
                    if video_url:
                        ...
                        if ok:
                            ...
                            print(f"✅ Reels IG [{slot_idx+1}/5]: {nome}")
