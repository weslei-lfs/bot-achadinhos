...
def supa_get(table, params=""):
    try:
        r = req.get(f"{SUPA_URL}/rest/v1/{table}?select=*{params}", headers=SUPA_HEADERS, timeout=10)
...
def get_reels():
    try:
        r = req.get(f"{SUPA_URL}/rest/v1/reels?select=*&order=id.desc", headers=SUPA_HEADERS, timeout=10)
        return r.json() if r.ok else []
    except: return []

def delete_reel(rid):
    try: req.delete(f"{SUPA_URL}/rest/v1/reels?id=eq.{rid}", headers=SUPA_HEADERS, timeout=10)
    except: pass

def redistribuir_reels():
    """Distribui reels uniformemente no período configurado para Instagram"""
    configs = get_config_db()
    ig_config = configs.get("ig_config", {})
    if isinstance(ig_config, str):
        try: ig_config = json.loads(ig_config)
        except: ig_config = {}

    hora_inicio_str = ig_config.get("inicio", "08:00")
    hora_fim_str    = ig_config.get("fim", "22:00")
    dias            = int(ig_config.get("dias", 7))
    ...
    reels = get_reels()
    if not reels:
        print("Nenhum reel para redistribuir")
        return

    total = len(reels)
    total_slots = minutos_por_dia * dias
    intervalo = total_slots / total

    hoje = datetime.now(BRASILIA)
    dt_base = hoje.replace(hour=hi_h, minute=hi_m, second=0, microsecond=0)

    print(f"Redistribuindo {total} reels | {hora_inicio_str}-{hora_fim_str} | {dias} dias")

    for i, reel in enumerate(reels):
        offset_min = i * intervalo
        ...
        horario_final = f"{data_str} {horario_str}"

        try:
            r = req.patch(
                f"{SUPA_URL}/rest/v1/reels?id=eq.{reel['id']}",
                headers=SUPA_HEADERS,
                json={"horarios": [horario_final]},
                timeout=10
            )
            print(f"⏰ Reel '{reel.get('nome','')}' → {horario_final}")
        except Exception as e:
            print(f"❌ Erro ao redistribuir reel: {e}")

    print(f"✅ {total} reels redistribuídos!")
...
def redistribuir_videos():
    """
    Distribui os vídeos para o Telegram dentro do período configurado.
    Igual aos produtos mas separado para não misturar os horários.
    """
    configs = get_config_db()
    periodo = configs.get("periodo", {})
    ...
    videos = get_videos()
    if not videos:
        print("Nenhum vídeo para redistribuir")
        return
    ...
    print(f"Redistribuindo {total} vídeos | {hora_inicio_str}-{hora_fim_str} | {dias} dias")
    ...
    for i, video in enumerate(videos):
        ...
        try:
            r = req.patch(
                f"{SUPA_URL}/rest/v1/videos?id=eq.{video['id']}",
                headers=SUPA_HEADERS,
                json={"horarios": [horario_final]},
                timeout=10
            )
            print(f"⏰ Vídeo '{video.get('nome','')}' → {horario_final} (status: {r.status_code})")
        except Exception as e:
            print(f"❌ Erro ao redistribuir vídeo: {e}")

    print(f"✅ {total} vídeos redistribuídos!")
...
def editar_video(vid):
    data = request.json
    if data.get("senha") != SENHA:
        return jsonify({"ok": False}), 401
    campos = {}
    for k in ["nome","preco","link","video","categoria","horarios","ig_hora"]:
        if k in data: campos[k] = data[k]
    try:
        req.patch(f"{SUPA_URL}/rest/v1/videos?id=eq.{vid}",
            headers=SUPA_HEADERS, json=campos, timeout=10)
        return jsonify({"ok": True})
    except Exception as e:
...
def salvar_configuracoes():
    ...
    ig_hora_inicio = data.get("ig_hora_inicio", "08:00")
    ...
    # Salva configuração de vídeos do Instagram
    ig_config = data.get("ig_config", {"inicio": ig_hora_inicio, "fim": "22:00", "qtd": 5, "dias": 7})
    for chave, valor in [("ig_hora_inicio", {"hora": ig_hora_inicio}), ("ig_config", ig_config)]:
        r2 = req.patch(f"{SUPA_URL}/rest/v1/configuracoes?chave=eq.{chave}",
            headers=SUPA_HEADERS, json={"valor": valor}, timeout=10)
        if not r2.ok or r2.status_code == 404:
            req.post(f"{SUPA_URL}/rest/v1/configuracoes",
                headers=SUPA_HEADERS, json={"chave": chave, "valor": valor}, timeout=10)
...
def ig_status():
    ...
    try:
        ...
        data = r.json()
        if data.get("id"):
            return jsonify({"ok": True, "username": data.get("username",""), "id": data.get("id")})
        return jsonify({"ok": False, "erro": data.get("error", {}).get("message", "Token inválido")})
    except Exception as e:
        return jsonify({"ok": False, "erro": str(e)})

@app.route("/api/ig/renovar", methods=["POST"])
def ig_renovar():
    data = request.json
    if data.get("senha") != SENHA:
        return jsonify({"ok": False}), 401
...
def get_ig_token():
    """Busca token do Instagram salvo no Supabase"""
    try:
        r = req.get(
            f"{SUPA_URL}/rest/v1/configuracoes?chave=eq.{IG_TOKEN_KEY}&select=*",
        ...
        return os.environ.get("IG_TOKEN", "IGAAgguE5mWbxBZAGE5TzdnRHRuSENYcFBjcE56SE1KT2NEZA1NETldNbVNya0lrUk0yNmpnVjZAHaTQxWmtWbWVMa2JYaEZABZAnlESEIzRXpXN0l4ZAFBsMk10VW8tZA2FMQ2ZAmLTBjekRfY0JibXN5RGpmdWVja05hdUZA4WTFkdDBlUQZDZD")
    except:
...
def salvar_ig_token(token):
    ...
    try:
        valor = {"token": token, "atualizado_em": datetime.now(BRASILIA).isoformat()}
        r = req.patch(
            f"{SUPA_URL}/rest/v1/configuracoes?chave=eq.{IG_TOKEN_KEY}",
            headers=SUPA_HEADERS, json={"valor": valor}, timeout=10
        )
        if not r.ok or r.status_code == 404:
            req.post(
                f"{SUPA_URL}/rest/v1/configuracoes",
                headers=SUPA_HEADERS,
                json={"chave": IG_TOKEN_KEY, "valor": valor},
                timeout=10
            )
        print("✅ Token Instagram salvo!")
    except Exception as e:
        print(f"❌ Erro ao salvar token: {e}")

def renovar_ig_token():
    """Renova token do Instagram automaticamente"""
    token = get_ig_token()
    if not token:
        return
    try:
        ...
        if novo_token:
            salvar_ig_token(novo_token)
            print(f"✅ Token Instagram renovado! Expira em {data.get('expires_in', 'N/A')}s")
        else:
            print(f"❌ Erro ao renovar token: {data}")
    except Exception as e:
        print(f"❌ Erro renovação token: {e}")

def postar_reel_instagram(video_url, legenda):
    """Posta vídeo como Reels no Instagram"""
    token = get_ig_token()
    if not token:
        print("❌ Token Instagram não encontrado")
        return False
    try:
        # Passo 1: Criar container do Reels
        r1 = req.post(
        ...
        if not container_id:
            print(f"❌ Erro ao criar container IG: {data1}")
            return False

        print(f"📸 Container IG criado: {container_id}")
        ...
        for tentativa in range(10):
            ...
            print(f"⏳ Status IG: {status} (tentativa {tentativa+1})")
            if status == "FINISHED":
                break
            if status == "ERROR":
                print("❌ Erro no processamento do vídeo no Instagram")
                return False
        ...
        data2 = r2.json()
        if data2.get("id"):
            print(f"✅ Reels publicado no Instagram! ID: {data2['id']}")
            return True
        else:
            print(f"❌ Erro ao publicar: {data2}")
            return False
...
def ig_token_loop():
    """Renova token a cada 50 dias automaticamente"""
    while True:
        try:
            time.sleep(50 * 24 * 60 * 60)  # 50 dias
            print("🔄 Renovando token Instagram...")
            renovar_ig_token()
        except Exception as e:
            print(f"❌ ig_token_loop: {e}")
            time.sleep(3600)


def calcular_horarios_ig():
    """
    Calcula os horários do dia para postagem de Reels no Instagram.
    Usa as configurações salvas no banco (inicio, fim, qtd).
    """
    configs = get_config_db()
    ig_config = configs.get("ig_config", {})
    if isinstance(ig_config, str):
        ig_config = json.loads(ig_config)

    hora_ini = ig_config.get("inicio", "08:00")
    hora_fim = ig_config.get("fim", "22:00")
    qtd = int(ig_config.get("qtd", 5))

    if not hora_ini or ":" not in str(hora_ini): hora_ini = "08:00"
    if not hora_fim or ":" not in str(hora_fim): hora_fim = "22:00"

    hi_h, hi_m = map(int, hora_ini.split(":"))
    ...
    intervalo = (fim_min - inicio_min) // (qtd - 1) if qtd > 1 else 0

    horarios = []
    for i in range(qtd):
        min_total = inicio_min + (i * intervalo)
        h = min_total // 60
        m = min_total % 60
        if h < 24:
...
def ig_reels_loop():
    """
    Posta 5 Reels por dia no Instagram.
    - Horários calculados automaticamente entre hora_inicio e 22:00
    - Vídeos novos têm prioridade (entram na frente da fila)
    - Ciclo infinito: quando acabar todos recomeça do zero
    """
    while True:
        try:
            ...
            horarios_dia = calcular_horarios_ig()
            enviados = get_enviados()

            if hora_atual not in horarios_dia:
                time.sleep(60)
                continue

            slot_idx = horarios_dia.index(hora_atual)
            chave_slot = f"ig_slot_{hoje}_{slot_idx}"

            if chave_slot in enviados:
                time.sleep(60)
                continue

            # Ordena vídeos: novos primeiro (maior id = mais recente)
            videos_ordenados = sorted(videos, key=lambda v: v.get("id", 0), reverse=True)

            # Conta quantos posts IG já foram feitos no total (ciclo global)
            total_posts_ig = sum(1 for k in enviados.keys() if k.startswith("ig_slot_"))
            total_videos = len(videos_ordenados)
            idx = total_posts_ig % total_videos
            video = videos_ordenados[idx]
            ...
            if not video_url:
                # Marca como enviado mesmo sem URL para não travar o ciclo
                enviados[chave_slot] = True
            ...
            legenda_ig = (
                ...
                f"#achadinhos #shopee #oferta #promoção #achadinhosshopee #economia #compras"
            )

            print(f"📸 Postando Reels IG [{slot_idx+1}/5] às {hora_atual}: {nome}")
            ok = postar_reel_instagram(video_url, legenda_ig)
            if ok:
                enviados[chave_slot] = True
                save_enviados(enviados)
                print(f"✅ Reels IG [{slot_idx+1}/5] postado: {nome}")

        except Exception as e:
            print(f"❌ ig_reels_loop: {e}")
        time.sleep(60)


def reels_loop():
    """
    Loop que envia Reels para o Instagram nos horários distribuídos.
    Ciclo infinito — quando acabar recomeça do zero.
    """
    while True:
        try:
            agora = datetime.now(BRASILIA)
            ...
            for reel in reels:
                ...
                for h in horarios:
                    ...
                    chave = f"reel_{rid}_{hoje_ymd}_{h_str}"
                    if deve_enviar and chave not in enviados:
                        video_url = reel.get("video","")
                        nome = reel.get("nome","")
                        ...
                        if video_url:
                            legenda = (
                                ...
                                f"💰 Por apenas R$ {preco}\n\n"
                                f"🛒 Link na bio ou acesse:\n{link}\n\n"
                                f"#achadinhos #shopee #oferta #promoção #achadinhosshopee #economia"
                            )
                            print(f"📸 Postando Reel IG: {nome}")
                            ok = postar_reel_instagram(video_url, legenda)
                            if ok:
                                enviados[chave] = True
                                save_enviados(enviados)
                                print(f"✅ Reel postado: {nome}")
                        else:
                            # Sem vídeo, marca como enviado para não travar
                            enviados[chave] = True
                            save_enviados(enviados)

            # Verifica ciclo — reinicia se passou do último horário
            todos_reels = get_reels()
            if todos_reels:
                ultimas = []
                for r in todos_reels:
                    hs = r.get("horarios",[])
                ...
                if ultimas:
                    ...
                    if " " in ultima:
                        ...
                        if agora.day > dia_ult or (agora.day == dia_ult and hora_str > hora_ult):
                            print("🔄 Ciclo de Reels completo! Reiniciando...")
                            redistribuir_reels()

        except Exception as e:
            print(f"❌ reels_loop: {e}")
        time.sleep(60)
...
def adicionar_reel():
    ...
    if data.get("senha") != SENHA:
        return jsonify({"ok": False}), 401
    categorias = get_categorias_db()
    cat_manual = data.get("categoria", "")
    categoria = cat_manual if cat_manual else detectar_categoria(data.get("nome", ""), categorias)
    reel = {
        "id": int(datetime.now(BRASILIA).timestamp() * 1000),
        "nome": data.get("nome", ""),
        "preco": data.get("preco", ""),
    ...
    req.post(f"{SUPA_URL}/rest/v1/reels", headers=SUPA_HEADERS, json=reel, timeout=10)
    time.sleep(1)
    redistribuir_reels()
...
def editar_reel(rid):
    ...
    if data.get("senha") != SENHA:
        return jsonify({"ok": False}), 401
    campos = {}
    for k in ["nome","preco","link","video","categoria","horarios"]:
        if k in data: campos[k] = data[k]
    try:
        req.patch(f"{SUPA_URL}/rest/v1/reels?id=eq.{rid}",
            headers=SUPA_HEADERS, json=campos, timeout=10)
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "erro": str(e)}), 500
...
def enviar_boas_vindas(nome):
    frase = MSGS_BV[bv_idx[0] % len(MSGS_BV)].format(nome=nome)
    bv_idx[0] += 1
    try:
        req.post(f"https://api.telegram.org/bot{TOKEN}/sendMessage",
            json={"chat_id": GRUPO_ID, "text": frase, "parse_mode": "Markdown"}, timeout=15)
        print(f"👋 Boas-vindas: {nome}")
    except Exception as e:
        print(f"❌ Boas-vindas: {e}")
...
def iniciar_threads():
    ...
    threading.Thread(target=processar_commands, daemon=True).start()
    threading.Thread(target=relatorio_semanal, daemon=True).start()
    threading.Thread(target=reels_loop, daemon=True).start()
    threading.Thread(target=ig_token_loop, daemon=True).start()
    # Salva token inicial do Instagram se não existir
    try:
        token_existente = get_ig_token()
    ...
    print("🤖 Bot Achadinhos iniciado! Todas as threads ativas.")
